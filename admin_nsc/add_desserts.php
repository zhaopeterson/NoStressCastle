...
$page_title = 'Add a Dessert Like Pies, Donuts, Cookies etc';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {	
	...
	if (empty($add_product_errors)) { // If everything's OK.
		...
		$q = 'INSERT INTO non_cake_products (category_id, name, description, image, price, stock) VALUES (?, ?, ?, ?, ?, ?)';
...
<form enctype="multipart/form-data" action="add_desserts.php" method="post" accept-charset="utf-8">
	...
	<fieldset><legend>Fill out the form to add a non-coffee product to the catalog. All fields are required.</legend>
		<div class="field"><label for="category"><strong>Category</strong></label><br /><select name="category"<?php if (array_key_exists('category', $add_product_errors)) echo ' class="error"'; ?>>
			...
			$q = 'SELECT id, category FROM non_cake_categories ORDER BY category ASC';		
			...
			<p class="field"><label for="name"><strong>Name</strong></label><br /><?php create_form_input('name', 'text', $add_product_errors); ?></p>
		
			<p class="field"><label for="price"><strong>Price</strong></label><br /><?php create_form_input('price', 'text', $add_product_errors); ?> <small>Without the dollar sign.</small></p>
	
			<p class="field"><label for="stock"><strong>Initial Quantity in Stock</strong></label><br /><?php create_form_input('stock', 'text', $add_product_errors); ?></p>
			
			<p class="field"><label for="description"><strong>Description</strong></label><br /><?php create_form_input('description', 'textarea', $add_product_errors); ?></p>

			<p class="field"><label for="image"><strong>Image</strong></label><br /><?php
