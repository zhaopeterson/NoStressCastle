...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {	
	...
	if (mysqli_num_rows($r) == 1) {
		...
		if ($order_total > 0) {
			...
			require_once( '../../private/gateway_setup_admin.php');
			...
			if ($response_array[0] == 1) {
				...
				$q = 'UPDATE cake_products AS cp, order_contents AS oc SET cp.stock=cp.stock-oc.quantity WHERE cp.id=oc.product_id AND oc.product_type="cake" AND oc.order_id=' . $order_id;
				$r = mysqli_query($dbc, $q);
				$q = 'UPDATE non_cake_products AS ncp, order_contents AS oc SET ncp.stock=ncp.stock-oc.quantity WHERE ncp.id=oc.product_id AND oc.product_type="other" AND oc.order_id=' . $order_id;
				$r = mysqli_query($dbc, $q);
...
// Define the query:
$q = 'SELECT total, shipping, credit_card_number, DATE_FORMAT(order_date, "%a %b %e, %Y at %h:%i%p") AS od, email, CONCAT(last_name, ", ", first_name) AS name, CONCAT_WS(" ", address1, address2, city, state, zip) AS address, phone, customer_id, CONCAT_WS(" - ", ncc.category, ncp.name) AS item, ncp.stock, quantity, price_per, DATE_FORMAT(ship_date, "%b %e, %Y") AS sd FROM orders AS o INNER JOIN customers AS c ON (o.customer_id = c.id) INNER JOIN order_contents AS oc ON (oc.order_id = o.id) INNER JOIN non_cake_products AS ncp ON (oc.product_id = ncp.id AND oc.product_type="other") INNER JOIN non_cake_categories AS ncc ON (ncc.id = ncp.category_id) WHERE o.id=' . $order_id . '
UNION 
SELECT total, shipping, credit_card_number, DATE_FORMAT(order_date, "%a %b %e, %Y at %l:%i%p"), email, CONCAT(last_name, ", ", first_name), CONCAT_WS(" ", address1, address2, city, state, zip), phone, customer_id, CONCAT_WS(" - ", cc.category, s.size, cp.cake_flavor, cp.cake_filling) AS item, cp.stock, quantity, price_per, DATE_FORMAT(ship_date, "%b %e, %Y") FROM orders AS o INNER JOIN customers AS c ON (o.customer_id = c.id) INNER JOIN order_contents AS oc ON (oc.order_id = o.id) INNER JOIN cake_products AS cp ON (oc.product_id = cp.id AND oc.product_type="cake") INNER JOIN sizes AS s ON (s.id=cp.size_id) INNER JOIN cake_categories AS cc ON (cc.id=cp.category_id) WHERE o.id=' . $order_id;
